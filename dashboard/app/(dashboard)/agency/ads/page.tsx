    ...
    <div className="grid grid-cols-3 gap-2 mt-2">
      {designs.map(d => (
        <div
          key={d.id}
          onClick={() => onSelect(d)}
          className="border border-white/[0.08] rounded-lg p-1.5 cursor-pointer hover:border-[#00FF88]/40 transition-colors"
        >
          <div
          ...
          />
          <p className="text-[9px] text-[#4A5568] mt-1 truncate">{d.design_type}</p>
        </div>
      ))}
    </div>
  );
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-xl bg-space-card border border-white/[0.06] p-4">
            <div className="flex items-center gap-2 mb-1">
              <Megaphone className="h-3.5 w-3.5 text-[#4A5568]" />
              <p className="text-xs text-[#4A5568]">Total campañas</p>
            </div>
            <p className="text-2xl font-bold text-[#00FF88]">{totalCampaigns}</p>
          </div>
          <div className="rounded-xl bg-space-card border border-white/[0.06] p-4">
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle2 className="h-3.5 w-3.5 text-[#4A5568]" />
              <p className="text-xs text-[#4A5568]">Activas</p>
            </div>
            <p className="text-2xl font-bold text-green-400">{activeCampaigns}</p>
          </div>
          <div className="rounded-xl bg-space-card border border-white/[0.06] p-4">
            <div className="flex items-center gap-2 mb-1">
              <DollarSign className="h-3.5 w-3.5 text-[#4A5568]" />
              <p className="text-xs text-[#4A5568]">Presupuesto activo / mes</p>
            </div>
        ...
        <div className="rounded-xl bg-space-card border border-white/[0.06] p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-xs text-[#4A5568] shrink-0">Cliente</label>
              <select
                value={selectedClientId}
                onChange={e => setSelectedClientId(e.target.value)}
                className="h-8 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-xs px-2 focus:outline-none focus:border-[#00FF88]/40 min-w-[160px]"
              >
            ...
            {selectedClient && (
              selectedClient.meta_connected ? (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="flex items-center gap-1.5 text-xs text-[#00FF88] font-semibold">
                    <span className="w-2 h-2 rounded-full bg-[#00FF88]" />
                    Meta conectado
                  </span>
                  <span className="text-xs text-[#4A5568] font-mono">{selectedClient.meta_ad_account_id}</span>
                  ...
                  <button
                    ...
                    disabled={refreshing}
                    className="flex items-center gap-1.5 text-xs px-3 h-7 rounded-lg border border-[#00FF88]/30 text-[#00FF88] bg-[#00FF88]/[0.08] hover:bg-[#00FF88]/15 transition-colors disabled:opacity-50"
                  >
                    {refreshing
          ...
          {syncCampMsg && (
            <p className={cn("text-xs", syncCampMsg.startsWith('Error') ? "text-red-400" : "text-[#00FF88]")}>
              {syncCampMsg}
            </p>
          )}
        </div>
        ...
        <div className="rounded-xl bg-space-card border border-white/[0.06] p-4">
          <div className="flex items-center gap-2 mb-3">
            <FileSpreadsheet className="h-4 w-4 text-[#00FF88]" />
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="h-8 rounded-lg bg-white/[0.04] border border-white/[0.08] text-[#A0AEC0] text-xs px-2 focus:outline-none focus:border-[#00FF88]/40"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs px-3 h-8 rounded-lg border border-[#00FF88]/30 text-[#00FF88] bg-[#00FF88]/[0.08] hover:bg-[#00FF88]/15 transition-colors disabled:opacity-50"
            >
              {exporting
          ...
          {syncMsg && (
            <p className={cn("mt-2 text-xs", syncMsg.startsWith('Error') ? "text-red-400" : "text-[#00FF88]")}>
              {syncMsg}
            </p>
          )}
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              {[
                ...
                <div key={label} className="text-center">
                  <p className="text-sm font-bold text-[#00FF88]">{count}</p>
                  <p className="text-[10px] text-[#4A5568]">{label}</p>
                </div>
              ))}
              <p className="text-[10px] text-[#4A5568] self-end">
              ...
              <button
                ...
                className={cn(
                  ...
                  statusFilter === f.id
                    ...
                    : "border-white/[0.06] text-[#4A5568] hover:text-white hover:border-white/[0.12]"
                )}
              >
                {f.label} ({f.count})
              </button>
          ...
          {analysis && (
            <div className="rounded-xl border border-purple-500/20 bg-purple-500/[0.05] p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setAnalysis("")} className="text-[#4A5568] hover:text-white text-xs">✕</button>
              </div>
              <p className="text-sm text-[#A0AEC0] leading-relaxed whitespace-pre-wrap">{analysis}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {filtered.map(camp => {
              ...
              return (
                <div
                  key={camp.id}
                  className="rounded-xl bg-space-card border border-white/[0.06] p-5 hover:border-[#00FF88]/20 transition-colors"
                >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#4A5568] mt-0.5">
                          ...
                          {camp.meta_campaign_id
                            ? <span className="ml-1 font-mono text-[#4A5568]">· #{camp.meta_campaign_id}</span>
                            : null}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-[#A0AEC0]">
                    ...
                    {camp.platform && (
                      <span className="rounded-md bg-white/[0.04] px-2 py-0.5">{camp.platform}</span>
                    )}
                    {(Number(m.ad_count) > 0) && (
                    ...
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <button
                          ...
                          className="flex items-center gap-1 text-[10px] text-[#4A5568] hover:text-[#A0AEC0] uppercase tracking-wider transition-colors"
                        >
                          Ad Copies
                          {isExpanded
                      ...
                      <div
                        className={cn(
                          "rounded-lg bg-black/20 border border-white/[0.04] p-3 text-xs text-[#A0AEC0] whitespace-pre-wrap font-mono leading-relaxed transition-all",
                  ...
                  {Boolean(m.design_id) && (() => {
                    ...
                    return linkedSvg ? (
                      <div className="mt-3 rounded-xl overflow-hidden border border-white/[0.06] [&_svg]:w-full [&_svg]:h-auto [&_svg]:block max-w-[200px]">
                        {/* eslint-disable-next-line react/no-danger */}
                    ...
                    return (
                      <div className="mt-3 p-3 rounded-xl bg-black/20 border border-white/[0.04]">
                        <div className="grid grid-cols-4 gap-2">
                          {[
                            { label: "Impresiones", value: fmtNum(s.impressions, 'count'),   color: "text-[#00FF88]"  },
                  ...
                  {camp.platform === "Meta Ads" && (camp.meta_campaign_id || (m.meta_campaign_id as string)) && (
                    <div className="mt-3 pt-3 border-t border-white/[0.04] flex items-center gap-1.5 flex-wrap">
                      <select
                        value={statsDatePreset}
                        onChange={e => setStatsDatePreset(e.target.value)}
                        className="h-6 rounded-md bg-white/[0.04] border border-white/[0.08] text-[#A0AEC0] text-[10px] px-1.5"
                      >
                        <option value="today">Hoy</option>
                        <option value="yesterday">Ayer</option>
                        <option value="last_7d">7 días</option>
          ...
          <DialogContent className="bg-space-card border-white/[0.08] text-white max-w-lg">
            ...
            {loadingAccounts ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-sm text-[#4A5568]">Cargando cuentas...</span>
              </div>
            ) : (
              <div className="space-y-4 py-2">
                {/* Ad accounts */}
                <div>
                  <label className="text-xs text-[#4A5568] mb-2 block">Cuenta publicitaria</label>
                  {availableAccounts.length === 0 ? (
                    <p className="text-xs text-[#4A5568]">No se encontraron cuentas publicitarias.</p>
                  ) : (
                        ...
                        <div
                          ...
                          className={cn(
                            ...
                            selAccountId === acc.account_id
                              ? "border-[#00FF88]/40 bg-[#00FF88]/[0.05]"
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-[11px] text-[#4A5568] font-mono mt-0.5">ID: {acc.account_id} · {acc.currency}</p>
                            </div>
                            {selAccountId === acc.account_id && (
                              <span className="text-[11px] text-[#00FF88] font-semibold">✓ Seleccionada</span>
                            )}
                          </div>
                        </div>
                      ))}
            ...
            <DialogFooter className="gap-2">
              <Button variant="ghost" onClick={() => setShowAccountPicker(false)} className="text-[#4A5568]">
                Cancelar
              </Button>
              <Button
          ...
          <div className="space-y-4 py-2">
                      ...
                      <SelectItem key={o} value={o} className="text-white hover:bg-white/[0.06]">
                        <div>
                          ...
                          {OBJECTIVE_DESCRIPTIONS[o] && (
                            <div className="text-[10px] text-[#4A5568]">{OBJECTIVE_DESCRIPTIONS[o]}</div>
                          )}
                        </div>
                      </SelectItem>
                    ))}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-[#4A5568]">Presupuesto MXN</label>
                <div className="flex rounded-lg overflow-hidden border border-white/[0.08] text-[10px]">
                  {["Mensual", "Diario"].map(t => (
                    <button key={t} type="button"
                      ...
                      className={`px-2.5 py-1 transition-colors ${form.budget_type === t ? "bg-[#00FF88] text-black font-semibold" : "bg-white/[0.04] text-[#4A5568] hover:text-white"}`}
                    >{t}</button>
                  ))}
                </div>
              </div>
            ...
            {isMetaAdsForm && (
              <div className="rounded-xl border border-[#00FF88]/15 bg-[#00FF88]/[0.03] p-3">
                <p className="text-[11px] text-[#4A5568] leading-relaxed">
                  ...
                  <span className="text-[#00FF88] font-medium">Sincronizar campañas</span> para importarla aquí con sus estadísticas automáticamente.
                </p>
              </div>
            )}
            ...
            <div className="rounded-xl border border-white/[0.08] p-4 space-y-3">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        copied
                          ...
                          : "text-[#A0AEC0] border-white/[0.08] hover:text-white"
                      )}
                    >
                      {copied
              ...
              {adCopies ? (
                <div className="rounded-lg bg-black/20 p-3 text-xs text-[#A0AEC0] whitespace-pre-wrap max-h-64 overflow-y-auto font-mono leading-relaxed">
                  {adCopies}
                </div>
              ) : (
            ...
            {form.client_id && (
              <div className="space-y-3">
                <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider">
                  Diseño para el anuncio <span className="text-[#4A5568] normal-case font-normal">(opcional)</span>
                </p>
                {clientDesigns.length === 0 ? (
                  <p className="text-xs text-[#4A5568]">
                    No hay diseños con imagen para este cliente.{" "}
                    <a href="/agency/design" className="text-[#00FF88] hover:underline">Crear diseño primero →</a>
                  </p>
                ) : (
                    ...
                    {clientDesigns.map(d => (
                      ...
                      >
                        ...
                        <div className="px-2 py-1 bg-black/20">
                          <p className="text-[10px] text-[#4A5568] truncate">{d.design_type?.replace(/_/g, " ")}</p>
                        </div>
                      </div>
                    ))}
                  </div>
