...
const BLANK_FORM = {
  ...
  brand_values: "", primary_color: "#00FF88", secondary_color: "#0F0F1A",
      ...
      {type === "textarea" ? (
        ...
        <div className="flex gap-2 items-center">
          ...
          <Input value={form[k] ?? ""} onChange={e => set(k, e.target.value)}
            className="bg-white/[0.04] border-white/[0.08] text-white font-mono text-sm flex-1" />
        </div>
      ) : (
        <Input value={form[k] ?? ""} onChange={e => set(k, e.target.value)}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {/* Form */}
        <div className="rounded-xl bg-space-card border border-white/[0.06] p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-[#4A5568] mb-1 block">Tono de comunicación</label>
          ...
          <div>
            <p className="text-xs text-[#4A5568] mb-2 font-semibold uppercase tracking-wider">Paleta de colores</p>
          ...
          <Button
            ...
            className="w-full bg-[#00FF88] text-black hover:bg-[#00FF88]/90 font-bold gap-2 py-3"
          >
            {generating ? (
        ...
        {manuals.length > 0 && (
          <div className="space-y-3">
            <p className="text-xs text-[#4A5568] uppercase tracking-wider font-semibold">Manuales guardados</p>
            {manuals.map(m => {
                ...
                <div key={m.id} className="rounded-xl border border-white/[0.06] overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4 bg-space-card">
                    <button onClick={() => setExpanded(e => ({ ...e, [m.id]: !isOpen }))}
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#4A5568]">{m.clients?.name} · {new Date(m.created_at).toLocaleDateString("es-MX")}</p>
                      </div>
                    </button>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {m.pdf_url && (
                        <a href={m.pdf_url} target="_blank" rel="noopener noreferrer"
                          className="p-1.5 rounded hover:bg-blue-500/10 text-[#4A5568] hover:text-blue-400 transition-colors" title="Descargar PDF">
                          <Download className="h-3.5 w-3.5" />
                        </a>
                      )}
                      <button onClick={() => regenerate(m)} title="Regenerar"
                        className="p-1.5 rounded hover:bg-white/[0.06] text-[#4A5568] hover:text-[#00FF88] transition-colors">
                  ...
                  {isOpen && m.content && (
                    <div className="px-5 py-4 bg-black/10 border-t border-white/[0.04]">
                      <pre className="whitespace-pre-wrap text-xs text-[#A0AEC0] font-sans leading-relaxed">{m.content}</pre>
                    </div>
                  )}
                </div>
              );
        ...
        <DialogContent className="bg-space-card border-white/[0.08] text-white max-w-3xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="rounded-xl bg-black/20 border border-white/[0.04] p-5">
              <pre className="text-xs text-[#A0AEC0] whitespace-pre-wrap font-sans leading-relaxed">{preview?.content}</pre>
            </div>
          </div>
          <DialogFooter className="mt-4 gap-2">
            ...
            <Button variant="ghost" onClick={() => preview?.content && copyContent(preview.content)} className="text-[#4A5568] gap-2">
              {copied ? <Check className="h-4 w-4 text-[#00FF88]" /> : <Copy className="h-4 w-4" />} Copiar
            </Button>
            <Button onClick={() => setPreview(null)} className="bg-[#00FF88] text-black font-semibold">Cerrar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
