  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {STATUSES.map(status => (
            <div key={status} className="rounded-xl bg-space-card border border-white/[0.06] p-4 text-center">
              <p className="text-[11px] text-[#4A5568] uppercase tracking-wider mb-1.5">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.length === 0 && (
              <p className="col-span-2 text-center text-[#4A5568] py-12">
            ...
            {filtered.map(d => {
              ...
              return (
                <div
                  key={d.id}
                  className="rounded-xl bg-space-card border border-white/[0.06] p-5 hover:border-[#00FF88]/20 transition-colors flex flex-col gap-3"
                >
                  {/* Card header */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-[#4A5568] mt-0.5 truncate">
                          {d.clients?.name}
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            {STATUSES.map(s => (
                              <button
                                ...
                                className={cn(
                                  ...
                                  d.status === s ? "text-[#00FF88]" : "text-[#A0AEC0]"
                                )}
                              >
                      ...
                      {d.pdf_url && (
                        <a
                          ...
                          className="p-1.5 rounded-lg hover:bg-blue-500/10 text-[#4A5568] hover:text-blue-400 transition-colors"
                          title="Descargar PDF"
                        >
                      ...
                      <button
                        onClick={() => openEdit(d)}
                        className="p-1.5 rounded-lg hover:bg-white/[0.06] text-[#4A5568] hover:text-white transition-colors"
                        title="Editar"
                      >
                  ...
                  {!d.canva_url && (
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="h-7 text-xs bg-white/[0.03] border-white/[0.06] text-white placeholder:text-[#4A5568] flex-1"
                      />
                      <Button
                        size="sm"
                        disabled={!canvaInputs[d.id]?.trim() || canvaSaving[d.id]}
                        onClick={() => saveCanvaUrl(d)}
                        className="h-7 px-2 bg-[#00FF88]/10 text-[#00FF88] hover:bg-[#00FF88]/20 border border-[#00FF88]/20 text-xs"
                      >
                        {canvaSaving[d.id]
                          ? <Loader2 className="h-3 w-3 animate-spin" />
                  ...
                  {d.canva_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-1 text-xs text-[#00FF88] hover:underline w-fit"
                    >
                  ...
                  {d.brief && (
                    <div>
                      <button
                        ...
                        className="flex items-center gap-1 text-[10px] text-[#4A5568] hover:text-[#A0AEC0] transition-colors"
                      >
                        {briefOpen
                      ...
                      {briefOpen && (
                        <div className="mt-2 rounded-lg bg-white/[0.03] border border-white/[0.04] p-3 max-h-48 overflow-y-auto">
                          <p className="text-xs text-[#A0AEC0] whitespace-pre-wrap leading-relaxed">{d.brief}</p>
                        </div>
                      )}
                      {!briefOpen && (
                        <p className="mt-1 text-xs text-[#4A5568] line-clamp-2 leading-relaxed">{d.brief}</p>
                      )}
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-lg border border-white/[0.08] bg-white/[0.03] text-[#4A5568] hover:text-[#A0AEC0] transition-colors disabled:opacity-50"
                        >
                          {cardGenImage[d.id]
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-space-card border-white/[0.08] text-white max-w-2xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-[#4A5568] mb-1.5 block">
            ...
            {brandManual && !brandLoading && (
              <div className="rounded-lg border border-[#00FF88]/30 bg-[#00FF88]/[0.05] p-3 flex items-start gap-2.5">
                <BookOpen className="h-4 w-4 text-[#00FF88] shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-medium text-[#00FF88]">
                    ✓ Manual de marca detectado — colores y estilo aplicados
                  </p>
                  <p className="text-[11px] text-[#A0AEC0] mt-0.5">{brandManual.title}</p>
                </div>
              </div>
            )}
            ...
            {/* Campaign */}
            <div>
              <label className="text-xs text-[#4A5568] mb-1.5 block">Campaña (opcional)</label>
              <Select
              ...
              >
                ...
                <SelectContent className="bg-space-card border-white/[0.08]">
                  <SelectItem value="__none__" className="text-[#4A5568] hover:bg-white/[0.06]">
                    Sin campaña
                  </SelectItem>
              ...
              {form.client_id && clientCampaigns.length === 0 && (
                <p className="text-[11px] text-[#4A5568] mt-1">No hay campañas para este cliente.</p>
              )}
            </div>

            {/* Dimension presets */}
            <div>
              <label className="text-xs text-[#4A5568] mb-1.5 block">Dimensiones del canvas</label>
              <Select
              ...
              {customDims ? (
                <div className="flex items-center gap-2 mt-2">
                  ...
                  />
                  <span className="text-[#4A5568] text-sm shrink-0">×</span>
                  <Input
                    type="number"
                    value={designHeight}
                ...
                <p className="text-[11px] text-[#4A5568] mt-1">{designWidth} × {designHeight} px</p>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs text-[#4A5568]">Imágenes de referencia (máx. 3)</label>
                {refUrls.length < 3 && (
            ...
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                className="h-7 px-3 text-[#00FF88] hover:text-[#00FF88] hover:bg-[#00FF88]/10 disabled:opacity-40 gap-1.5 text-xs"
              >
                {generating
                  ? <><Loader2 className="h-3 w-3 animate-spin" /> Generando...</>
            ...
            <Textarea
              ...
              className="bg-white/[0.04] border-white/[0.08] text-white placeholder:text-[#4A5568] resize-none text-sm font-mono leading-relaxed"
            />

            {/* Row: status */}
            <div>
              <label className="text-xs text-[#4A5568] mb-1.5 block">Estado</label>
              <Select
                value={form.status}
            ...
            {/* Canva URL in modal */}
            <div>
              <label className="text-xs text-[#4A5568] mb-1.5 block">URL de Canva (opcional)</label>
              <Input
            ...
            {form.brief && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs text-[#4A5568]">Imagen del diseño (AI)</label>
                  <Button
                    size="sm"
                    variant="ghost"
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              className="bg-[#00FF88] text-black hover:bg-[#00FF88]/90 font-semibold min-w-[90px]"
            >
        ...
        <Dialog open={!!imageModal} onOpenChange={() => setImageModal(null)}>
          <DialogContent className="bg-[#07070f] border-white/[0.08] text-white max-w-3xl w-full p-4">
            <DialogHeader>
              <DialogTitle className="text-purple-300 text-sm flex items-center gap-2">
                ...
                {imageModal.clients?.name && (
                  <span className="text-[#4A5568] font-normal">— {imageModal.clients.name}{imageModal.clients.company ? ` / ${imageModal.clients.company}` : ''}</span>
                )}
              </DialogTitle>
            </DialogHeader>
            <div className="w-full rounded-xl border border-white/[0.06] overflow-hidden [&_svg]:w-full [&_svg]:h-auto [&_svg]:block">
            ...
            <DialogFooter className="gap-2">
              ...
              <Button variant="ghost" onClick={() => setImageModal(null)} className="text-[#4A5568] hover:text-white">
                Cerrar
              </Button>
            </DialogFooter>
          </DialogContent>
