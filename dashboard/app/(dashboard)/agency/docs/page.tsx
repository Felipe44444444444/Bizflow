...
const DOC_TYPES = [
  { key: "contrato",   step: 1, label: "Contrato de Servicios",    icon: ShieldCheck,  color: "#00FF88" },
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {clientId && (
          <>
            ...
            {config && !showConfigForm && (
              <div className="flex items-center justify-between rounded-xl border border-[#00FF88]/20 bg-[#00FF88]/5 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm text-[#00FF88] font-medium">Configuración de proyecto cargada</p>
                  <span className="text-xs text-[#4A5568]">— {config.razon_social}</span>
                </div>
                <button
                  onClick={() => setShowConfigForm(true)}
                  className="text-xs text-[#4A5568] hover:text-white flex items-center gap-1 transition-colors"
                >
            ...
            {/* Config form */}
            {showConfigForm && (
              <div className="rounded-xl bg-space-card border border-[#00FF88]/20 p-6 space-y-6">
                <div className="flex items-center gap-2">
                  <Settings className="h-4 w-4 text-[#00FF88]" />
                  <p className="font-semibold text-white">Configuración del proyecto</p>
                  <span className="text-xs text-[#4A5568]">Requerida para generar documentos</span>
                </div>

                {/* Client data */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Datos del cliente</p>
                ...
                {/* Services */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Servicios contratados *</p>
                    ...
                    {SERVICES.map(svc => {
                      ...
                      return (
                        <label key={svc} className={cn(
                          ...
                          checked ? "border-[#00FF88]/30 bg-[#00FF88]/5 text-white" : "border-white/[0.06] text-[#A0AEC0] hover:border-white/[0.12]"
                        )}>
                          <input
                            type="checkbox"
                            checked={checked}
                            ...
                            className="mt-0.5 accent-[#00FF88] shrink-0"
                          />
                          {svc}
                        </label>
                      );
                ...
                {/* Economic conditions */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Condiciones económicas</p>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="text-xs text-[#4A5568] mb-1 block">Método de pago</label>
                    ...
                    <div>
                      <label className="text-xs text-[#4A5568] mb-1 block">Día de pago mensual</label>
                      <Select value={configForm.payment_day} onValueChange={v => setConfigForm(f => ({ ...f, payment_day: v }))}>
                        <SelectTrigger className="bg-white/[0.04] border-white/[0.08] text-white">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="bg-space-card border-white/[0.08] max-h-48 overflow-y-auto">
                    ...
                    <div>
                      <label className="text-xs text-[#4A5568] mb-1 block">Duración del contrato</label>
                    ...
                    <div>
                      <label className="text-xs text-[#4A5568] mb-1 block">Penalización por cancelación</label>
                ...
                {/* Date */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Fecha de inicio</p>
                  <div className="grid grid-cols-2 gap-3">
                ...
                {/* Agency data */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Datos de la agencia</p>
                  <div className="grid grid-cols-2 gap-3">
                ...
                {/* Bank data */}
                <div>
                  <p className="text-xs font-semibold text-[#A0AEC0] uppercase tracking-wider mb-3">Datos bancarios de la agencia (para facturas)</p>
                  <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex gap-3 pt-2">
                  {config && (
                    <Button variant="ghost" onClick={() => setShowConfigForm(false)} className="text-[#4A5568]">
                      Cancelar
                    </Button>
                  )}
                  <Button
        ...
        {/* ── Document cards ─────────────────────────────────────────────────── */}
        <div>
          <p className="text-xs text-[#4A5568] uppercase tracking-wider font-semibold mb-4">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    isDone
                      ...
                      : "bg-space-card border-white/[0.06] hover:border-white/[0.1]"
                  )}
                >
                  <div className="flex items-start gap-3 mb-4">
                    <div className={cn(
                      "p-2 rounded-lg shrink-0",
                      isDone ? "bg-[#00FF88]/10" : "bg-white/[0.04]"
                    )} style={{ color: isDone ? "#00FF88" : doc.color }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {isLoading ? null : isDone ? (
                          <Badge className="text-[9px] bg-[#00FF88]/10 text-[#00FF88] border-[#00FF88]/20">Generado</Badge>
                        ) : (
                          <Badge className="text-[9px] bg-white/[0.04] text-[#4A5568] border-white/[0.06]">Pendiente</Badge>
                        )}
                      </div>
                  ...
                  {isDone ? (
                    <div className="flex gap-2 flex-wrap">
                      <Button
                        ...
                        className="gap-1.5 bg-[#00FF88]/10 text-[#00FF88] hover:bg-[#00FF88]/20 border border-[#00FF88]/20 text-xs h-8"
                        variant="ghost"
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          copied === `doc-${doc.step}`
                            ...
                            : "text-[#4A5568] border-white/[0.06] hover:text-white"
                        )}
                      >
                      ...
                      <button
                        ...
                        disabled={isGen}
                        className="flex items-center gap-1.5 text-xs px-2.5 h-8 rounded-lg border border-white/[0.06] text-[#4A5568] hover:text-white hover:border-white/[0.12] transition-colors disabled:opacity-40"
                      >
        ...
        <div className="rounded-xl bg-space-card border border-[#00FF88]/20 p-5 space-y-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={t.is_active
                        ? "bg-[#00FF88]/10 text-[#00FF88] border-[#00FF88]/20 text-[9px]"
          ...
          {activeTokens.length > 0 && (
            <div className="rounded-lg bg-black/20 border border-white/[0.04] p-3">
              ...
              <div className="flex items-center gap-2">
                <code className="text-xs text-[#00FF88] flex-1 truncate font-mono">
      ...
      <Dialog open={!!preview} onOpenChange={() => setPreview(null)}>
        <DialogContent className="bg-space-card border-white/[0.08] text-white max-w-3xl max-h-[85vh] flex flex-col">
          ...
          <DialogFooter className="gap-2 mt-4">
            ...
            <Button onClick={() => setPreview(null)} className="bg-[#00FF88] text-black font-semibold">
              Cerrar
            </Button>
          </DialogFooter>
        </DialogContent>
