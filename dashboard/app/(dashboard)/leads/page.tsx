      ...
      <div className="flex-1 p-6 space-y-6 overflow-auto">
        ...
        {metrics && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-xl border border-neon-cyan/[0.08] bg-space-card p-4">
              <p className="text-xs text-[#4A5568] mb-1">Leads este mes</p>
              <div className="flex items-end gap-2">
                <p className="text-2xl font-bold text-white">{metrics.total_this_month}</p>
                {metrics.variation_pct !== null && (
                  <span className={`text-xs mb-0.5 flex items-center gap-0.5 ${metrics.variation_pct >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {metrics.variation_pct >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                    {Math.abs(metrics.variation_pct)}%
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-neon-cyan/[0.08] bg-space-card p-4">
              <p className="text-xs text-[#4A5568] mb-2">Por canal</p>
              <div className="flex flex-wrap gap-1.5">
                {Object.entries(metrics.by_canal).map(([ch, n]) => (
                  <span key={ch} className="text-xs px-2 py-0.5 rounded-full bg-space-el border border-neon-cyan/[0.08] text-[#A0AEC0]">
                    {CANAL_CONFIG[ch]?.icon || "📡"} {n}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-neon-cyan/[0.08] bg-space-card p-4">
              <p className="text-xs text-[#4A5568] mb-1">De anuncios</p>
              <div className="flex items-center gap-2">
                <Megaphone className="h-4 w-4 text-neon-cyan" />
                <p className="text-2xl font-bold text-white">{metrics.total_ads}</p>
              </div>
            </div>

            <div className="rounded-xl border border-neon-cyan/[0.08] bg-space-card p-4">
              <p className="text-xs text-[#4A5568] mb-1">Orgánicos</p>
              <div className="flex items-center gap-2">
                <Globe className="h-4 w-4 text-[#4A5568]" />
                <p className="text-2xl font-bold text-white">{metrics.total_organic}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 rounded-xl border border-neon-cyan/[0.08] overflow-hidden">
            {loading ? (
              <div className="flex justify-center py-20"><Loader2 className="h-5 w-5 animate-spin text-[#4A5568]" /></div>
            ) : leads.length === 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-neon-cyan/[0.04]">
                  {leads.map(lead => (
                    <tr
                      key={lead.id}
                      className={`hover:bg-neon-cyan/[0.03] cursor-pointer transition-colors ${selected?.id === lead.id ? 'bg-neon-cyan/[0.05]' : ''}`}
          ...
          {selected && (
            <div className="w-80 shrink-0 rounded-xl border border-neon-cyan/[0.08] bg-space-card overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between p-4 border-b border-neon-cyan/[0.06]">
                ...
                <button onClick={() => setSelected(null)} className="text-[#4A5568] hover:text-white p-1">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {(selected.ad_name || selected.campaign_name) && (
                  <div className="space-y-1.5">
                    ...
                    {selected.campaign_name && <p className="text-xs text-[#4A5568]">Campaña: {selected.campaign_name}</p>}
                  </div>
                )}
                ...
                <div>
                  ...
                  {messages.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {messages.map(msg => (
                        <div key={msg.id} className={`text-xs rounded-lg p-2 ${msg.direction === 'inbound' ? 'bg-space-el text-[#A0AEC0]' : 'bg-neon-cyan/10 text-neon-cyan'}`}>
                          {msg.content}
                        </div>
                      ))}
                    </div>
