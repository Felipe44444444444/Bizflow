  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {/* Token banner */}
        {newToken && (
          <div className="relative rounded-xl border border-[#00D4AA]/30 bg-[#00D4AA]/10 p-4">
            <div className="flex items-start gap-3">
              <div className="shrink-0 p-2 rounded-lg bg-[#00D4AA]/20">
                <Key className="h-5 w-5 text-[#00D4AA]" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#00D4AA] mb-1">
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 bg-black/30 rounded-lg px-3 py-2 text-sm font-mono text-[#00D4AA] tracking-widest border border-[#00D4AA]/20">
                    {newToken.token}
                  </code>
                  <Button
                    size="sm"
              ...
              <button
                ...
                className="shrink-0 p-1 rounded-lg text-[#64748B] hover:text-white hover:bg-white/[0.06] transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total clientes", value: clients.length, icon: Briefcase, color: "text-[#00D4AA]" },
            ...
            <div key={label} className="rounded-xl bg-[#0D0D1F] border border-[#1A1A35] p-4 flex items-center gap-4">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-[#1A1A35] overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1A1A35] bg-[#080812]">
              ...
              <tbody>
                ...
                {filtered.map(c => (
                  <tr key={c.id} className="border-b border-[#1A1A35] hover:bg-[#080812] transition-colors">
                    ...
                    <td className="px-4 py-3 text-[#94A3B8]">{c.plan ?? "—"}</td>
                    <td className="px-4 py-3 text-[#00D4AA] font-medium">
                      ...
                      {c.meta_connected ? (
                        <div className="flex flex-col gap-0.5">
                          ...
                          {c.meta_ad_account_id && (
                            <span className="text-[10px] text-[#64748B] font-mono">{c.meta_ad_account_id}</span>
                          )}
                        </div>
                      ) : (
                        <button
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-[#64748B] hover:text-white transition-colors">
      ...
      {metaSelector && (
        <Dialog open={!!metaSelector} onOpenChange={() => setMetaSelector(null)}>
          <DialogContent className="bg-[#0D0D1F] border-white/[0.08] text-white max-w-md">
            <DialogHeader>
            ...
            <div className="space-y-4 py-2">
              {metaSelector.adAccounts.length > 0 && (
                <div>
                  <label className="text-xs text-[#64748B] mb-1.5 block">Cuenta publicitaria</label>
                  <select
                    value={selAdAccount}
            ...
            <DialogFooter>
              <Button variant="ghost" onClick={() => setMetaSelector(null)} className="text-[#64748B]">Cancelar</Button>
              <Button
                disabled={savingMeta}
                onClick={async () => {
                  setSavingMeta(true);
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setSaveError(null); }}>
        <DialogContent className="bg-[#0D0D1F] border-white/[0.08] text-white max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            ...
            <div>
              <label className="text-xs text-[#64748B] mb-1 block">MRR mensual ($)</label>
              <Input
                type="number"
                value={form.monthly_revenue ?? ""}
                onChange={e => setForm(f => ({ ...f, monthly_revenue: Number(e.target.value) || undefined }))}
                className="bg-[#0D0D1F] border-white/[0.08] text-white"
              />
            </div>
            <div>
            ...
            {editing && (
              <div className="col-span-2 space-y-3 pt-3 border-t border-[#1A1A35]">
                  ...
                  <div className="rounded-lg border border-blue-500/20 bg-blue-500/[0.05] p-3 space-y-1.5">
                    ...
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg border border-white/[0.08] text-[#94A3B8] hover:text-white hover:bg-white/[0.06] transition-colors"
                      >
            ...
            {editing && (
              <div className="col-span-2 space-y-3 pt-3 border-t border-[#1A1A35]">
                ...
                <div className="space-y-2">
                  <div>
                    <label className="text-xs text-[#64748B] mb-1 block">URL de Supabase</label>
                    <Input
                      value={sbUrl}
                      onChange={e => setSbUrl(e.target.value)}
                      placeholder="https://xxxxx.supabase.co"
                      className="bg-[#0D0D1F] border-white/[0.08] text-white placeholder:text-[#64748B] font-mono text-xs"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-[#64748B] mb-1 block">Anon Key</label>
                    <Input
                      value={sbAnonKey}
                  ...
                  <div>
                    <label className="text-xs text-[#64748B] mb-1 block">Service Role Key</label>
                    <Input
                      value={sbServiceKey}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    ...
                    className="h-7 px-3 text-xs text-[#00D4AA] hover:text-[#00D4AA] hover:bg-[#00D4AA]/10 border border-[#00D4AA]/20 gap-1.5"
                  >
                    {sbSaving
                ...
                {sbStatusMsg && (
                  <p className={cn("text-xs", sbStatus === 'ok' ? "text-green-400" : sbStatus === 'error' ? "text-red-400" : "text-[#94A3B8]")}>
                    {sbStatusMsg}
                  </p>
                )}
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={save} disabled={saving || !form.name} className="bg-[#00D4AA] text-black hover:bg-[#00D4AA]/90 font-semibold">
