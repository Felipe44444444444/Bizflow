      ...
      <div>
        ...
        {multi ? (
          <Textarea
            ...
            rows={2}
            className="bg-[#0D0D1F] border-white/[0.08] text-white placeholder:text-[#64748B] text-sm resize-none"
          />
        ) : (
          <Input
            type={type}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="rounded-xl bg-[#0D0D1F] border border-[#1A1A35] p-4">
          <div className="flex items-end gap-4">
            <div className="flex-1">
              ...
              {loading ? (
                <div className="h-10 rounded-lg bg-[#0D0D1F] animate-pulse" />
              ) : (
          ...
          <>
            ...
            <div className="rounded-xl bg-[#0D0D1F] border border-[#1A1A35] p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-medium text-[#94A3B8]">Progreso de onboarding</p>
                <p className="text-sm font-bold text-[#00D4AA]">{completedCount}/6</p>
              </div>
                ...
                {ONBOARDING_STEPS.map(s => (
                  <div
                    ...
                    className={cn(
                      ...
                      getDoc(s.number)?.status === "completado" ? "bg-[#00D4AA]" : "bg-white/[0.08]"
                    )}
                  />
                ))}
              </div>
                ...
                {ONBOARDING_STEPS.map(s => (
                  <span key={s.number}
                    className={cn("text-[9px] font-medium",
                      getDoc(s.number)?.status === "completado" ? "text-[#00D4AA]" : "text-[#64748B]"
                    )}>
                    {s.number}
                  </span>
                ))}
            ...
            <div className="rounded-xl border border-[#00D4AA]/20 bg-[#00D4AA]/[0.03] p-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <Button
                  ...
                  className="bg-[#00D4AA] text-black hover:bg-[#00D4AA]/90 font-semibold gap-2 shrink-0"
                >
                  {genAllProgress
              ...
              {genAllProgress && (
                <div className="mt-3 space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-[#94A3B8]">{genAllProgress.stepName}...</span>
                    <span className="text-[#00D4AA]">{genAllProgress.current}/{genAllProgress.total}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                    <div className="h-full rounded-full bg-[#00D4AA] transition-all duration-300"
                ...
                {ONBOARDING_STEPS.map(step => {
                  ...
                  return (
                    <div key={step.number} className={cn(
                      ...
                      isCompleted ? "border-[#00D4AA]/20 bg-[#00D4AA]/[0.03]" : "border-[#1A1A35] bg-[#0D0D1F]"
                    )}>
                      <div className="flex items-start gap-4">
                        ...
                        <div className={cn(
                          ...
                          isCompleted
                            ...
                            : "bg-[#0D0D1F] text-[#64748B] border-white/[0.08]"
                        )}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            ...
                            {step.number === 1 && selectedClient.contract_signed && (
                              <>
                                <Badge className="text-[10px] border bg-[#00D4AA]/10 text-[#00D4AA] border-[#00D4AA]/20 shrink-0">
                                ...
                                {selectedClient.signature_data && (
                                  <img
                                    ...
                                    alt="Firma"
                                    className="h-6 w-auto max-w-[60px] rounded border border-[#00D4AA]/20 bg-white/5 object-contain shrink-0"
                                  />
                                )}
                              </>
                            )}
                          ...
                          {isDiscovery && isCompleted && !discoveryEditing && (
                            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5">
                              {Object.entries(discoveryForm)
                                ...
                                .slice(0, 4)
                                .map(([k, v]) => (
                                  <p key={k} className="text-[10px] text-[#94A3B8] truncate">
                                    <span className="text-[#64748B]">{k.replace(/_/g, " ")}: </span>{v}
                                  </p>
                                ))}
                            </div>
                          )}
                          ...
                          {isDiscovery ? (
                            ...
                            isCompleted && !discoveryEditing ? (
                              <button
                                ...
                                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-[#0D0D1F] text-[#94A3B8] hover:bg-white/[0.08] hover:text-white transition-colors"
                              >
                            ...
                            <>
                              ...
                              >
                                {copied === step.number
                                  ? <><Check className="h-3.5 w-3.5 text-[#00D4AA]" />Copiado</>
                              ...
                              <button
                                ...
                                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-[#0D0D1F] text-[#64748B] hover:bg-white/[0.08] hover:text-white transition-colors disabled:opacity-40"
                              >
                                {isGenerating
                            ...
                            <button
                              onClick={() => generateStep(step.number)}
                              disabled={isGenerating || !!genAllProgress}
                              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#00D4AA]/10 text-[#00D4AA] hover:bg-[#00D4AA]/20 transition-colors disabled:opacity-40"
                            >
                              {isGenerating
                                ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Generando...</>
                      ...
                      {isDiscovery && discoveryEditing && (
                        <div className="mt-4 pt-4 border-t border-[#1A1A35] space-y-3">
        ...
        {!loading && !clients.length && (
          <p className="text-center text-[#64748B] py-12">No hay clientes registrados.</p>
        )}
      </div>
      ...
      <Dialog open={!!viewDoc} onOpenChange={() => setViewDoc(null)}>
        <DialogContent
          className="bg-[#0D0D1F] border-white/[0.08] text-white max-w-2xl flex flex-col"
        ...
        >
          ...
          <div className="overflow-y-auto flex-1 mt-2 pr-1">
            <pre className="text-sm text-[#94A3B8] whitespace-pre-wrap leading-relaxed font-sans">
              {viewDoc?.content}
            </pre>
          </div>
          <div className="flex justify-end gap-2 mt-4 shrink-0 pt-2 border-t border-[#1A1A35]">
            <Button
              variant="ghost"
              onClick={() => viewDoc && copyDoc(viewDoc)}
              className="text-[#94A3B8] gap-2"
            >
              <Copy className="h-4 w-4" /> Copiar contenido
            </Button>
            <Button onClick={() => setViewDoc(null)} className="bg-[#00D4AA] text-black hover:bg-[#00D4AA]/90">
              Cerrar
            </Button>
          </div>
        </DialogContent>
